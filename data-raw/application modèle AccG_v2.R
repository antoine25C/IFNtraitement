...
N_ha <- raster(paste0(sig_dendro_dir,raster_dir,'/','N_pred.tif'))
G_ha <- raster(paste0(sig_dendro_dir,raster_dir,'/','G_pred.tif'))
G_res<- raster(paste0(sig_dendro_dir,raster_dir,'/','GR_pred.tif'))
G_feu<- raster(paste0(sig_dendro_dir,raster_dir,'/','GF_pred.tif'))
G_GB<- raster(paste0(sig_dendro_dir,raster_dir,'/','GGB_pred.tif'))
Dg <- raster(paste0(sig_dendro_dir,raster_dir,'/','Dg_pred.tif'))
...
BD <- brick(N_ha, G_ha, G_res, G_feu, G_GB, Dg, type_melange_code, alt, pente, expo, ser_raster, rad, prec)
names(BD) <- c("Nha", "Gha", "G_res", "G_feu", "G_GB", "Dg", "type_melange_code", "alt", "pent2", "expo", "codeser_num", "rad", "prec")
